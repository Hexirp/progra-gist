...
(** * Pre
    ...
    第一に、表記法（演算子などの書き方）を事前に定義する。
    これらは実際に何を意味するか定義されてはいないが、
    優先順位や結合性は定義されている。
    どの小単位 (Module) に書かれているかによって
    [2 + 3 + 4 * 0] が [(2 + 3) + (4 * 0)] となったり、
    [((2 + 3) + 4) * 0] となったりすると読みづらいためだ。
    また、最初から [_ + _] を自然数同士の足し算と決めたりすると、
    整数同士の足し算と解釈したいときなどに困る。
    ちなみに、書かれている場所によってどの解釈を選ぶかは視野 (scope) の仕組みによる。

    第二に、視野 (scope) を定義する。
    例えば、 [(Empty + Unit)%type] という式があったとする。
    このとき、百分率記号は、右側の式が解釈されるとき、
    その内の表記法が type という視野の中から探されることを示す。
    どの表記法がどの視野に入るかは、表記法の意味を定義するときに定義できる。
    同じ表記法でも、視野によって意味が異なることがある。
...
Module Pre.
 ...
 (** ** 述語論理の記号
     ...
     優先順位は [_ -> _] 、 [_ <-> _] 、 [_ \/ _] 、 [_ /\ _] 、 [~ _] の順に高くなって行く。
     含意記号は特別な設定がされていて、右側のみ優先順位が低い。
     これによって、 [P -> _] と書かれた時、 [_] の部分が分けられて認識されることはない。
